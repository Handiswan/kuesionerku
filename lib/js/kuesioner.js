var kuesioner = {
	...
	'simpanPilihan' : function(id) {
	    ...
	    r.done(function(data) {
			...
			var li = document.getElementById('li_' + id);
		        ...
		        button.setAttribute(
              'onclick', "kuesioner.likert.editPilihanTersimpan('" + id + "')"
            );
			...
			// lalu buat del button
		        ...
		        del.setAttribute('class', 'btn btn-danger');
		        del.setAttribute(
              'onclick', "kuesioner.likert.hapusPilihanTersimpan('" + id + "')"
            );
	...
	next.setAttribute('onclick', 'tampilkanFormDataIsianResponden()');
	...
	for (var x = 1; x < 999; x++) {
		...
		*/
		...
		i.id = "p_" + x;
		i.type = "text";
		i.setAttribute('onblur', 'kuesioner.simpanPertanyaan(' + x + ')');
		i.placeholder = "Pertanyaan...";
		li.appendChild(i);
		li.appendChild(document.createTextNode(' '));
		var b = _e('button');
		b.className = "btn btn-danger";
		b.id = 'h_' + x;
		b.setAttribute('onclick', 'kuesioner.hapusPertanyaanTersimpan(' + x + ')');
		b.innerHTML = "Hapus";
		b.setAttribute('style','display:none');
		li.appendChild(b);
		li.appendChild(_e('br'));
